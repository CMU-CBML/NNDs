...
NeuronGrowth::NeuronGrowth(){
	...
	source_coeff		= 0.05;		// source_coeff
}
...
void NeuronGrowth::InitializeProblemNG(const int n_bz, vector<Vertex2D>& cpts, vector<Vertex2D> prev_cpts, vector<vector<float>> &NGvars, vector<array<float, 2>> &seed)
{
	...
	if (n == 0) { // Initialize
		...
		for (int i = 0; i < prev_cpts.size(); i++) {
			if (abs(remainder(prev_cpts[i].coor[0],1)) != 0.5)
				prev_cpts[i].coor[0] = round(prev_cpts[i].coor[0]);
			if (abs(remainder(prev_cpts[i].coor[1],1)) != 0.5)
		...
		for (int i = 0; i < cpts.size(); i++) {
			...
			if (SearchPair(prev_cpts, x, y , ind)) {
			// if (SearchPair(prev_cpts, cpts[i].coor[0], cpts[i].coor[1], ind)) {
				...
				&& SearchPair(prev_cpts, floorf(x), round(y), indDown)
				&& SearchPair(prev_cpts, floorf(x)+1, round(y), indUp)) {
				...
				&& SearchPair(prev_cpts, round(x), floorf(y), indDown) 
				&& SearchPair(prev_cpts, round(x), floorf(y)+1, indUp)) {
					phi[i] = (NGvars[0][indDown] + NGvars[0][indUp])/2;
					syn[i] = (NGvars[1][indDown] + NGvars[1][indUp])/2;
					tub[i] = (NGvars[2][indDown] + NGvars[2][indUp])/2;
					// theta[i] = max(NGvars[3][indDown], NGvars[3][indUp]);
				...
				&& SearchPair(prev_cpts, floorf(x), floorf(y), indDown) 
				&& SearchPair(prev_cpts, floorf(x)+1, floorf(y), indUp)
				&& SearchPair(prev_cpts, floorf(x), floorf(y), indLeft)
				&& SearchPair(prev_cpts, floorf(x), floorf(y)+1, indRight)) {
...
// void NeuronGrowth::EvaluateOrientation(const int nen, const vector<float> &Nx, const vector<array<float, 2>> &dNdx, const vector<float> elePhi, const vector<float> eleTheta,  float& eleEpsilon, float& eleEpsilonP)
// {
// 	for (int i = 0; i < nen; i++) {
// 		eleEpsilon += epsilonb * (1.0 + delta * cos(aniso * (atan2(elePhi[i] * dNdx[i][0], elePhi[i] * dNdx[i][1]) - eleTheta[i] * Nx[i])));
// 		eleEpsilonP += -epsilonb * (aniso * delta * sin(aniso * (atan2(elePhi[i] * dNdx[i][0], elePhi[i] * dNdx[i][1]) - eleTheta[i] * Nx[i])));
// 	}
// }
		...
		for (int i = 0; i < Gpt.size(); i++) {
			for (int j = 0; j < Gpt.size(); j++) {
				...
				for (int m = 0; m < nen; m++) {
					...
					EVectorSolve_tub[m] += (vars_st[2] * vars_st[6] + (dt/2) * vars_st[11]) * Nx[m] * detJ;

					for (int n = 0; n < nen; n++) {
						if (judge_syn == 0)
							EMatrixSolve_syn[m][n] += (Nx[m] * Nx[n] + dt/2 * Dc * (dNdx[m][0] * dNdx[n][0] + dNdx[m][1] * dNdx[n][1])) * detJ;
						...
						EMatrixSolve_tub[m][n] += ((vars_st[0] * Nx[m] + vars_st[2] * Nx[m]) * Nx[n] - (dt/2) * (
							(- Diff * (vars_st[2] * (dNdx[m][0] * dNdx[n][0] + dNdx[m][1] * dNdx[n][1])))
							- (alphaT * (vars_st[2] * (dNdx[m][0] + dNdx[n][1]) + Nx[m] * (vars_st[3] + vars_st[4])) * Nx[n])
							- (betaT * (vars_st[2] * Nx[m]) * Nx[n]))
							) * detJ;
...
void NeuronGrowth::DetectTipsSquare(vector<float> phi, vector<float> &tips)
{
	...
	int ind;
	float maxVal = 0;
	for (int i = (4*sz+5); i < (length-4*sz-4); i++) {
		tips[i] = 0;
		for (int j = -4; j < 4; j++) {
			for (int k = -4; k < 4; k++) {
				tips[i] += CellBoundary(phi[i+j*sz+k], 0.25);
			}
		}	
		if (tips[i] > 0)
			tips[i] = round(phi[i]) / tips[i];
		// std::cout << tips[i] << std::endl;
		// if (std::isnan(tips[i]))
		// 	tips[i] = 0;
		if (tips[i] > maxVal)
			maxVal = tips[i];
	...
	// for (int i = 1+sz; i < length-sz-1; i++) {
	// 	if (tips[i] == 1) {
	// 		tips[i-1] = 0; tips[i+1] = 0;
	// 		tips[i-sz] = 0; tips[i-sz-1] = 0; tips[i-sz+1] = 0;
	// 		tips[i+sz] = 0; tips[i+sz-1] = 0; tips[i+sz+1] = 0;
	// 	}
	// }
}

void NeuronGrowth::DetectTipsRectangle(vector<float> phi, vector<float> &tips, int NX, int NY)
{
	float threshold = 0.90;
	...
	for (int i = (5*NY+5); i < (length-4*NY-4); i++) {
		...
		if (CellBoundary(phi[i], 0.1) > 0) {
		// if (round(phi[i]) > 0) {
			for (int j = -4; j < 5; j++) {
				for (int k = -4; k < 5; k++) {
					tips[i] += CellBoundary(phi[i+j*(NY+1)+k], 0.1);
				}
			}	
			if (tips[i] > 0)
				tips[i] = CellBoundary(phi[i], 0.1) / tips[i];
				// tips[i] = round(phi[i]) / tips[i];
			if (std::isnan(tips[i]))
				tips[i] = 0;
			if (tips[i] > maxVal)
...
void NeuronGrowth::DetectTipsMulti(vector<float> id, int numNeuron, vector<float> &tips, int NX, int NY)
{
	...
	for (int i = (5*NY+5); i < (length-4*NY-4); i++) {
		tips[i] = 0;
		// if (CellBoundary(phi_in[i], 0.1) > 0) {
...
vector<float> NeuronGrowth::FindLocalMaximaInClusters(const vector<float>& matrix, int rows, int cols) {
	...
	for (const auto& cluster : clusters) {
		...
		if (maxPos.first != -1 && maxPos.second != -1) {
			...
			// localMaxima[maxPos.first * cols + maxPos.second - 2*cols + 2] = 1.0f;
			// localMaxima[maxPos.first * cols + maxPos.second - 2*cols + 1] = 1.0f;
			// localMaxima[maxPos.first * cols + maxPos.second - 2*cols] = 1.0f;
			// localMaxima[maxPos.first * cols + maxPos.second - 2*cols - 1] = 1.0f;
			// localMaxima[maxPos.first * cols + maxPos.second - 2*cols - 2] = 1.0f;

			// localMaxima[maxPos.first * cols + maxPos.second - cols + 2] = 1.0f;
			// localMaxima[maxPos.first * cols + maxPos.second - cols + 1] = 1.0f;
			// localMaxima[maxPos.first * cols + maxPos.second - cols] = 1.0f;
			// localMaxima[maxPos.first * cols + maxPos.second - cols - 1] = 1.0f;
			// localMaxima[maxPos.first * cols + maxPos.second - cols - 2] = 1.0f;

			// localMaxima[maxPos.first * cols + maxPos.second - 2] = 1.0f;
			// localMaxima[maxPos.first * cols + maxPos.second - 1] = 1.0f;
			// localMaxima[maxPos.first * cols + maxPos.second + 1] = 1.0f;
			// localMaxima[maxPos.first * cols + maxPos.second + 2] = 1.0f;
...
// Function to perform flood fill
void NeuronGrowth::IdentifyNeurons(vector<vector<int>>& neurons, vector<array<float, 2>> seed, int NX, int NY, int originX, int originY) 
{
	neurons = ConvertTo2DIntVector(phi, NX+1, NY+1);   
...
int RunNG(int n_bzmesh, vector<vector<int>> ele_process_in, vector<Vertex2D> cpts_initial, const vector<Element2D> tmesh_initial, vector<Vertex2D> &cpts, vector<Vertex2D> prev_cpts, const vector<Element2D> &tmesh, string path_in, string path_out, int &iter, int end_iter_in,
...
{	
	...
	NG.DetectTipsMulti(id, NG.numNeuron, tips, NX, NY);
	NG.tips = InterpolateVars(tips, cpts_initial, cpts, 0);	
	...
	while (iter <= NG.end_iter) {
		...
		if ((NG.n % 5 == 0) && (NG.n != 0)) {	
			...
			NG.DetectTipsMulti(id, NG.numNeuron, NG.tips, NX, NY);
			...
			distances = NG.CalculateGeodesicDistanceFromPoint(neurons, seed, originX, originY);
			vector<float> geodist = ConvertTo1DFloatVector(distances);
		...
		if ((NG.n % 25 == 0) && (NG.n != 0)) {	
			...
			// varName = "geodist_lf_running_";
			// geodist_lf = InterpolateVars(distances, cpts_initial, cpts, 1);
			// NG.VisualizeVTK_ControlMesh(cpts, tmesh, NG.n, path_out, geodist_lf, varName); // solution on control points
		...
		if ((NG.n % NG.expandCK_invl == 0) && (NG.n != 0)) {
			...
			if (expd_dir <= 3) {
				switch (expd_dir) {
					case 0: // left
						NX += 6;	originX -= 6;		break;
					case 1: // top
						NY += 6;				break;
					case 2: // right
						NX += 6;				break;
					case 3: // bottom
						NY += 6;	originY -= 6;		break;
				}
			}
