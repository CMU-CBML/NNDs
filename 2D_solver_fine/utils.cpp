...
vector<float> InterpolateVars_new(vector<float> input, vector<Vertex2D> cpts_initial, vector<Vertex2D> cpts, int type) 
{	
	vector<float> output;
	output.resize(cpts.size());
	...
	for (int i = 0; i < cpts_initial.size(); i++) {
		...
		// max_x = max(max_x, cpts_initial[i].coor[0]);
		// max_y = max(max_y, cpts_initial[i].coor[1]);
		// min_x = min(min_x, cpts_initial[i].coor[0]);
		// min_y = min(min_y, cpts_initial[i].coor[1]);
	}
...
void ObtainRefineID_new(vector<float> phi, vector<Vertex2D> cpts, int NX, int NY, int originX, int originY, vector<int> &rfid, vector<int> &rftype){
