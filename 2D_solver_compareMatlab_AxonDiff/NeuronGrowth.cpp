		...
		for (int i = 0; i < Gpt.size(); i++) {
			for (int j = 0; j < Gpt.size(); j++) {
				...
				for (int m = 0; m < nen; m++) {
					...
					for (int n = 0; n < nen; n++) {
						if (judge_syn == 0)
							EMatrixSolve_syn[m][n] += (Nx[m] * Nx[n] + dt/4 /M_ratio * Dc * 2 * (dNdx[m][0] * dNdx[n][0] + dNdx[m][1] * dNdx[n][1])) * detJ;
...
vector<vector<int>> NeuronGrowth::ConvertTo2DIntVector(const vector<float> input, int NX, int NY) 
{
	...
	for (int i = 0; i < NX+1; i++) {
		...
		for (int j = 0; j < NY+1; j++) {
			...
			row.push_back(CellBoundary(input[k], 0.1));
			k++;
		}
		...
		if ((NG.n % 1 == 0) || (NG.n == 0)) {	
			...
			if (NG.n > 5000) {
				...
				for (size_t l = 0; l < geodist.size(); l++) {
					...
					if (geodist[l] > maxVal && geodist[l] != INF && tip[l] != 0) {
						maxVal = geodist[l];
						maxInd = l;
					}
				}
