...
void SingularPatchEval::ProjectPoints(vector<Vertex2D>& Cp,vector<Vertex2D>& C,int N)
{
	Cp.resize(2*N+8);
	...
	for(int i=0;i<2*N+8;i++)
	{
		...
		{
			Cp[i].coor[0]=Cp[i].coor[0]+C[j].coor[0]*eigen[N-3].iV[IX(i,j,2*N+8)];
			Cp[i].coor[1]=Cp[i].coor[1]+C[j].coor[1]*eigen[N-3].iV[IX(i,j,2*N+8)];
			Cp[i].coor[2]=Cp[i].coor[2]+C[j].coor[2]*eigen[N-3].iV[IX(i,j,2*N+8)];
		}
	}
}
